...
bot.on('message', (msg) => {
    ...
    if (msg.sticker) {
        var msgtext = "⭐️ 貼圖" + msg.sticker.set_name + '|' + msg.sticker.file_id + '|' + msg.sticker.emoji
    }
    ...
    var SendLog2Ch = "<code>[訊息]</code>" +
        ...
        "\n 發送時間：" + msg.date +
        "\n 訊息文字：" + msgtext + "</code>" +
        "\n<a href='t.me/" + msg.from.username + "'>#UserName_" + msg.from.username + "</a> #Name_" + msg.from.first_name + " #UserID_" + msg.from.id
    bot.sendMessage("-1001123393977", SendLog2Ch, { parse_mode: "HTML", disable_web_page_preview: true }).then((returnmsg) => {
        if (msg.sticker) {
    ...
    if (msg.text != undefined) {
        ...
        if (msg.text.toLowerCase().indexOf("询问") > -1 || msg.text.toLowerCase().indexOf("詢問") > -1) {
            ...
            if (msg.text.toLowerCase().indexOf("＃") > -1) {
                var text = "<b>錯誤 - Tag 無法被正常偵測</b>" +
                    "\n<a href='http://telegra.ph/%E5%A6%82%E4%BD%95%E6%AD%A3%E7%A2%BA%E7%9A%84-Tag-07-25'>查看正確的 #Tag 方式</a>";
            } else if (msg.text.toLowerCase().indexOf("#询问#") > -1 || msg.text.toLowerCase().indexOf("#詢問#") > -1) {
                var text = "<b>錯誤 - Tag 無法被正常偵測</b>" +
                    "\n<a href='http://telegra.ph/%E5%A6%82%E4%BD%95%E6%AD%A3%E7%A2%BA%E7%9A%84-Tag-07-25'>查看正確的 #Tag 方式</a>";
